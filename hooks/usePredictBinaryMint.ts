...
/**
 * DeepBook Predict — binary (Up/Down) position mint.
 *
 * Mints a binary position at a specific strike, paying from the caller's dUSDC.
 * Uses predict::get_trade_amounts (devInspect) to size contract quantity from the
 * target dUSDC spend, then deposit + mint in one PTB.
 */
...
export type PredictBinaryMintArgs = {
  oracleId: string;
  expiryMs: number;
  strikeUsd: number;
  isUp: boolean;
...
export function usePredictBinaryMint() {
  ...
  /** Use devInspect to price Q0 units, then back-solve the right quantity. */
  const sizeQuantity = useCallback(
    async (owner: string, a: PredictBinaryMintArgs): Promise<string> => {
      const tx = new Transaction();
      const key = tx.moveCall({
        target: a.isUp ? TARGETS.marketKeyUp : TARGETS.marketKeyDown,
        arguments: [
          tx.pure.id(a.oracleId),
          tx.pure.u64(a.expiryMs),
          tx.pure.u64(toStrikeU64(a.strikeUsd)),
        ],
      });
      tx.moveCall({
      ...
      if (!rv?.length) throw new Error("Could not price this strike (no live quote)");
      const askCost = Number(bcs.U64.parse(Uint8Array.from(rv[0][0])));
      if (askCost <= 0) throw new Error("Position not currently mintable");
      const qty = Math.floor((Number(toDusdcU64(a.amountDusdc)) * Q0) / askCost);
      if (qty <= 0) throw new Error("Amount too small for this position");
      return String(qty);
    },
    [client],
  );
